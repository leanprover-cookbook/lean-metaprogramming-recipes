...
-- Setup for example code paths and external assets (CSS/JS)
...
elab "load_asset_file " p:str : term => do
  let s ← IO.FS.readFile p.getString
  return mkStrLit s

def customCodeCss : CssFile := { 
  filename := "custom-code.css", 
  contents := load_asset_file "assets/cookbook-custom-style.css" 
}

def customJs : JsFile := { 
  filename := "custom.js", 
  contents := load_asset_file "assets/cookbook-custom.js", 
  sourceMap? := none 
}
